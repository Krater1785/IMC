...
class Trader:
    def __init__(self, ink_limit=20, ink_liquidate_quantity=5):  # Reduced limits to match exchange
        self.resin_prices = []
        self.resin_vwap = []
        self.kelp_prices = []
        self.kelp_vwap = []
        self.vwap_slope_threshold = 0.005
        self.vwap_diff_threshold = 0.3
        ...
        self.LIMIT = {
            ...
            Product.SQUID_INK: 20  # Added explicit limit for squid ink
        }
        ...
        self.params = {
            Product.SPREAD: {
                ...
                "target_position": 2,  # Reduced from 3 to be more conservative
            },
            Product.SPREAD2: {
    ...
    def validate_position(self, product, desired_quantity, current_position):
        position_limit = self.LIMIT.get(product, 0)
        if desired_quantity > 0:  # Buying
            return min(desired_quantity, position_limit - current_position)
        else:  # Selling
            return max(desired_quantity, -position_limit - current_position)

    def get_max_quantity(self, product, current_position, is_buy):
        position_limit = self.LIMIT.get(product, 0)
        if is_buy:
            return position_limit - current_position
        return -position_limit - current_position

    def squid_ink_strategy(self, product, order_depth, position, prices, vwap_list, timespan) -> list[Order]:
        orders = []
        current_position = position
        max_buy = self.get_max_quantity(product, current_position, True)
        max_sell = self.get_max_quantity(product, current_position, False)
        ...
        if order_depth.sell_orders and order_depth.buy_orders:
            ...
            if abs(slope) > self.vwap_slope_threshold:
                if slope > 0 and diff > self.vwap_diff_threshold:
                    quantity = min(calculate_buy_quantity(order_depth, best_ask, max_buy), max_buy)
                    if quantity > 0:
                        orders.append(Order(product, best_ask, quantity))

                elif slope < 0 and diff < -self.vwap_diff_threshold:
                    quantity = max(calculate_sell_quantity(order_depth, best_bid, max_sell), max_sell)
                    if quantity < 0:
                        orders.append(Order(product, best_bid, quantity))

            elif abs(slope) <= self.vwap_slope_threshold / 2:
                bollinger_band_width = 2 * np.std(prices[-timespan:]) if len(prices) >= timespan else 0
                upper_band = vwap + bollinger_band_width
                lower_band = vwap - bollinger_band_width

                if mid_price > upper_band:
                    quantity = max(calculate_sell_quantity(order_depth, best_bid), max_sell)
                    if quantity < 0:
                        orders.append(Order(product, best_bid, quantity))
    ...
    def execute_spread_orders(self, state:TradingState, basket_product, target_position, current_position, order_depths):
        ...
        # Calculate maximum possible quantity considering all component limits
        max_quantity = abs(target_position - current_position)
        for product, weight in basket_weights.items():
            limit = self.LIMIT.get(product, 0)
            current_pos = state.position.get(product, 0)
            if target_position > current_position:  # Buying basket = selling components
                max_for_component = (limit + current_pos) // weight
            else:  # Selling basket = buying components
                max_for_component = (limit - current_pos) // weight
        ...
        if buy_side:
            ...
            quantity = min(basket_volume, synthetic_volume, max_quantity)
    ...
    def spread_orders(self, order_depths, basket_product, basket_position, spread_data):
        ...
        if zscore > threshold and basket_position != -target_pos:
            return self.execute_spread_orders(state, basket_product, -target_pos, basket_position, order_depths)
    ...
    def spread2_orders(self, order_depths, basket_product, basket_position, spread_data):
        ...
        threshold = self.adaptive_threshold(spread_data["spread_history"], base_threshold=self.params[Product.SPREAD2]["zscore_threshold"])
    ...
    # ... [rest of the methods remain the same, but make sure to use validate_position() where needed]
    ...
    def run(self, state: TradingState):
        ...
        # Initialize position tracking for all products
        ...
        for basket in [Product.PICNIC_BASKET1]:
            component_orders = self.spread_orders(
                order_depths,
                basket,
                current_positions.get(basket, 0),
            ...
            if component_orders:
                for product, orders in component_orders.items():
                    ...
                    for order in orders:
                        valid_quantity = self.validate_position(product, order.quantity, current_pos)
                        if valid_quantity != 0:
                            validated_orders.append(Order(product, order.price, valid_quantity))
                            current_pos += valid_quantity  # Update running position total
                    result[product].extend(validated_orders)
        ...
        resin_orders = self.hybrid_orders(Product.RAINFOREST_RESIN, order_depths[Product.RAINFOREST_RESIN], 
                                        current_positions.get(Product.RAINFOREST_RESIN, 0), 
                                        self.resin_prices, self.resin_vwap, 10, 1, 3.5)
        kelp_orders = self.hybrid_orders(Product.KELP, order_depths[Product.KELP], 
                                       current_positions.get(Product.KELP, 0), 
                                       self.kelp_prices, self.kelp_vwap, 10, 1, 3.5)
