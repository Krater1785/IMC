...
class Trader:
    ...
    def take_best_orders(self, product: str,
        ...
        if order_depth.sell_orders:
            ...
            # Updated condition to match starfruit logic
            if (not prevent_adverse or abs(best_ask_amount) >= adverse_volume) and best_ask <= fair_value - take_width:
                ...
                if quantity > 0:
                    ...
                    # Update order depth like starfruit implementation
    ...
    def clear_position_order(self,
        ...
        fair_for_bid = round(fair_value - clear_width)  # Added clear_width
        fair_for_ask = round(fair_value + clear_width)  # Added clear_width
        
        # Rest of the implementation remains similar but uses the width-adjusted prices
        buy_quantity = self.LIMIT[product] - (position + buy_order_volume)
        sell_quantity = self.LIMIT[product] + (position - sell_order_volume)

        if position_after_take > 0:
            # Aggregate volume from all buy orders >= fair_for_ask
            clear_quantity = sum(
                volume
        ...
        if position_after_take < 0:
            # Aggregate volume from all sell orders <= fair_for_bid
            clear_quantity = sum(
                abs(volume)
